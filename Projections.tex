...
We give two explicit examples for $k=3$, but the same constructions work for any $k\ge 3$.
...
In the second example (Section \ref{sec:discs}), the convex subsets $C_1,C_2,C_3$ are isometric to the unit disc, and the composition $P\:C_1\z\to C_1$ rotates the disc by an arbitrary (in particular an irrational) angle.
...
Moreover, $|x_n-x_{n+1}| =o (\frac  {1} {\sqrt n})$ for any starting point $x\in X$ and the sequence $x_n \z= P^n (x)$.
(We denote by $|x_n-x_{n+1}|$ the distance between the points $x_n$ and $x_{n+1}$ in $X$.)
\end{thm}
...
Let $T$  be the composition of translation along $E$  of length $2\pi +10\cdot\epsilon$ and a given rotation~$R$.
...
If $R$ is irrational rotation, then $P$, as well as all its powers, are \emph{not} asymptotically regular.
